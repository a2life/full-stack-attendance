...
import { fetchMembers, createEventSheet, createMember } from '../api';
...
export default function NewEventSheet() {
  ...
  const handleAddMember = async () => {
    if (!newMemberId.trim() || !newMemberName.trim()) {
      setAddMemberError('Both Member ID and Name are required.');
      return;
    }
    if (attendance.some((a) => a.memberId === newMemberId.trim())) {
      setAddMemberError('A member with this ID is already on the sheet.');
      return;
    }
    setAddingMember(true);
    setAddMemberError('');
    try {
      const created = await createMember({ memberId: newMemberId.trim(), name: newMemberName.trim() });
      setAttendance((prev) => [
        ...prev,
        { memberId: created.memberId, memberName: created.name, present: false, remark: '' },
      ]);
      setNewMemberId('');
      setNewMemberName('');
  ...
  const handleSave = async () => {
    ...
    setError('');
    try {
      await createEventSheet({ eventName: eventName.trim(), eventDate: today, attendance });
      navigate('/sheets');
    } catch {
      ...
      <div className="new-sheet">
        <div className="sheet-header">
          <div>
            ...
            <p className="sheet-subtitle">Fill in the event details and mark attendance below.</p>
          </div>
        </div>
        ...
        <div className="sheet-meta-card">
          ...
          <div className="meta-row">
            ...
            <div className="meta-date">
              <span className="date-badge">{format(new Date(), 'EEEE, MMMM d, yyyy')}</span>
        ...
        <div className="attendance-section">
          <div className="attendance-header">
            <div className="attendance-title">
              <h2>Members</h2>
              <span className="count-badge">{presentCount} / {attendance.length} present</span>
            </div>
            <div className="bulk-actions">
              <button className="bulk-btn" onClick={() => markAll(true)}>Mark All Present</button>
              <button className="bulk-btn" onClick={() => markAll(false)}>Mark All Absent</button>
              <button
                  className="bulk-btn add-member-btn"
          ...
          {showAddMember && (
              <div className="add-member-panel" ref={addPanelRef}>
                <div className="add-member-fields">
                  <div className="add-field">
                    <label className="add-label">Member ID</label>
                    <input
                        className="add-input"
                        type="text"
                  ...
                  <div className="add-field add-field-name">
                    <label className="add-label">Full Name</label>
                    <input
                        className="add-input"
                        type="text"
                  ...
                  <button
                      className="add-member-confirm"
                      onClick={handleAddMember}
                      disabled={addingMember}
                  >
                    {addingMember ? 'Adding…' : 'Add to Sheet'}
                  </button>
                </div>
                {addMemberError && <p className="add-member-error">{addMemberError}</p>}
                <p className="add-member-note">This will also save the new member to the database.</p>
              </div>
          )}
          ...
          <div className="attendance-table-wrap">
            <table className="attendance-table">
              ...
              {attendance.map((entry, idx) => (
                  <tr key={entry.memberId} className={entry.present ? 'row-present' : ''}>
                    <td className="col-id">
